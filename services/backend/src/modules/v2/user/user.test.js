...
const { userOne, userTwo, admin, insertUsers } = require('../../../../tests/fixtures/user.fixture');
...
describe('User routes v2', () => {
  describe('POST /v2/users', () => {
    ...
    test('should return 201 and successfully create new user if data is ok', async () => {
      ...
      const dbUser = await getUserRepository().findById(res.body.id);
  ...
  describe('GET /v2/users/:userId', () => {
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).get(`/v2/users/${userOne.id}`).send().expect(httpStatus.UNAUTHORIZED);
    });
  ...
  describe('DELETE /v2/users/:userId', () => {
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).delete(`/v2/users/${userOne.id}`).send().expect(httpStatus.UNAUTHORIZED);
    });
  ...
  describe('PATCH /v2/users/:userId', () => {
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).patch(`/v2/users/${userOne.id}`).send(updateBody).expect(httpStatus.UNAUTHORIZED);
    });
